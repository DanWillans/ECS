...
TEST_CASE("Test ComponentArray")
{
  ...
  auto ValidCheckAndNext = [&]<typename T>(EntityID id, T val) {
...
TEST_CASE("Test ComponentManager")
{
  ...
  auto RegEntityAndCheck = [&]<typename T>(EntityID id, ComponentID<T> comp_id, T comp) {
    auto err = comp_manager.AddComponent<T>(id, comp);
    REQUIRE(err.Good());
  };
  ...
  auto ValidCheckAndNext = [&]<typename T>(EntityID entity_id, ComponentID<T>& comp_id, int val) {
