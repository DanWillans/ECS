...
int main(int argc, const char** argv)
{
  ...
  // Register components
  auto err = ecs_controller.RegisterComponent<Animals>();
  if (err) {
    auto err = ecs_controller.RegisterComponent<AnimalFood>();
  }
  if (err) {
    auto err = ecs_controller.RegisterComponent<AnimalHabitat>();
  }
  if (err) {
    auto err = ecs_controller.RegisterComponent<AnimalHairStyle>();
  }

  if (err) {
    // Create signature for our system
    SystemSignature animal_system_signature;
    animal_system_signature.SetComponent<Animals, AnimalFood, AnimalHabitat, AnimalHairStyle>();
    ...
    std::cout << "ECS error: " << err.Message() << "\n";
  }
